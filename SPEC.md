...
- **"You Pass"** (เพิ่ม 2026-07-23, แก้กติกาเดียวกันวัน) — ผ่านเมื่อ**ทุกคำตอบถูกครบทุกเกม นับเฉพาะหลังการตอบผิดครั้งล่าสุดของคำนั้น**: ตอบผิด (Again) 1 ครั้ง = **รีเซ็ตแถวของคำนั้นทั้งหมดทุกเกม เริ่มนับใหม่** ไม่ว่าผิดในเกมไหน — เป้าหมายคือวน loop จนกว่าจะ "ไม่ผิดเลย" ทั้งชุด (เช็คจาก `reviews_log` ผ่าน `domain/mastery.dart`) ขึ้นหน้าจอฉลองเต็มจอ "You Pass" **ครั้งเดียว** (setting `you_pass_shown`)
- **Fade-out คำที่แน่นแล้ว** (คู่กับกติกา reset) — คำที่ตอบถูกติดกันมาหลายรอบ (streak นับตั้งแต่ Again ล่าสุด) ถูก**ลดน้ำหนักการโผล่ใน loop ลงเรื่อย ๆ** (weight = 1/(1+streak), `weightedPracticeSample`) — กันไม่ให้หลังพลาดทีนึงต้องไล่เจอแต่คำง่ายซ้ำ ๆ กว่าจะถึงคำยาก ทำให้ progress ช้า
