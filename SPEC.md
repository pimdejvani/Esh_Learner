...
- **"You Pass"** (เพิ่ม 2026-07-23, กติกาสุดท้าย) — ผ่านเมื่อทำได้ **1 รอบสมบูรณ์: ทุกคำตอบถูกครบทุกเกม โดยไม่มีการตอบผิดแทรกเลยแม้แต่ครั้งเดียว** — ตอบผิด (Again) 1 ครั้ง**กับคำไหนก็ตาม = รีเซ็ตทั้งกระดาน (ทุกคำทุกเกม) เริ่มนับ 1 ใหม่** การนับมีไว้เพื่อจบรอบ clean รอบเดียวเท่านั้น (เช็คจาก `reviews_log`: นับเฉพาะคำตอบถูกหลัง Again ล่าสุดในระบบ ผ่าน `domain/mastery.dart`) ขึ้นหน้าจอฉลองเต็มจอ "You Pass" **ครั้งเดียว** (setting `you_pass_shown`)
